...
import ChickenContract, { defaultGasLimit } from "./ChickenContract";


function Main(props) {
    const { api, keyring } = useSubstrate();
    const { accountPair } = props;
    const chickenContract = ChickenContract(api);
    ...
    const transfer = () => {
        chickenContract.tx.transfer(0, defaultGasLimit, addressTo, amount).signAndSend(accountPair, (result) => {
            updateBalance();
        });
    }
    ...
    useEffect(() => {
        let unsubscribe;
        chickenContract.query.totalSupply(keyring.getPairs()[0].address, 0, defaultGasLimit).then((total) => {
    ...
    return (
        <Grid.Column>
            <h1>Transfer CHICKEN</h1>
            <Card.Group>
                <Card style={{ width: "calc(50% - 1em)", padding: "20px" }}>
                    <Statistic value={totalSupply} label={'Total CHICKEN'} />
                </Card>
                <Card style={{ width: "calc(50% - 1em)", padding: "20px" }}>
                    <Statistic value={balance} label={'CHICKEN in wallet'} />
                </Card>
            </Card.Group>
            <Divider hidden />
            <Card.Group>
...
export default function ChickenTransfer(props) {
